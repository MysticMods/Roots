...
public class MixinProjectileWeaponItem$WildwoodQuiver {
  ...
  private static void roots$UseAmmo(Inventory instance, ItemStack ammo, Operation<Void> original, @Local Player player) {
    // Important: even though the cycleComponent exists when you examine the item stack, if it evaluates to empty (i.e., count is 0, which is the case for this injection point to even reach), the components/patch that is consulted by `has` will be empty.
