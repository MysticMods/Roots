...
import epicsquid.roots.entity.EntityRitualLife;
...
public class RitualLife extends RitualBase {
  ...
  public void doEffect(World world, BlockPos pos) {
    List<EntityRitualLife> pastRituals = world
        .getEntitiesWithinAABB(EntityRitualLife.class, new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 100, pos.getZ() + 1));
    if (pastRituals.size() == 0 && !world.isRemote) {
      ...
      for (EntityRitualLife ritual : pastRituals) {
        ritual.getDataManager().set(EntityRitualLife.getLifetime(), duration + 20);
        ritual.getDataManager().setDirty(EntityRitualLife.getLifetime());
      }
    }
  }
}