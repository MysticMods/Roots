...
public class ArmorBaseModel<T extends LivingEntity> extends BipedModel<T> {
  ...
  public ArmorBaseModel(EquipmentSlotType slot) {
  ...
  public void prepareForRender(Entity entity) {
    ...
    if (living != null) {
      BipedModel.ArmPose mainPose = BipedModel.ArmPose.EMPTY;
      BipedModel.ArmPose offPose = BipedModel.ArmPose.EMPTY;
      if (living.getHeldItemMainhand() != null) {
        mainPose = BipedModel.ArmPose.ITEM;
        if (living.getItemInUseCount() > 0) {
          UseAction enumaction = living.getHeldItemMainhand().getUseAction();
          if (enumaction == UseAction.BLOCK) {
            mainPose = BipedModel.ArmPose.BLOCK;
          } else if (enumaction == UseAction.BOW) {
            mainPose = BipedModel.ArmPose.BOW_AND_ARROW;
          }
        }
      }
      if (living.getHeldItemOffhand() != null) {
        offPose = BipedModel.ArmPose.ITEM;
        if (living.getItemInUseCount() > 0) {
          UseAction enumaction1 = living.getHeldItemOffhand().getUseAction();
          if (enumaction1 == UseAction.BLOCK) {
            offPose = BipedModel.ArmPose.BLOCK;
          }
        }
      }
  ...
  public static void setRotation(RendererModel model, float x, float y, float z) {
