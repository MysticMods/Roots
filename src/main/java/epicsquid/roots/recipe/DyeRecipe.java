...
public class DyeRecipe extends IForgeRegistryEntry.Impl<IRecipe> implements IRecipe {
  ...
  public boolean matches(InventoryCrafting inv, World worldIn) {
    ...
    for (int i = 0; i < inv.getSizeInventory(); i++) {
      ...
      if (inSlot.getItem() == ModItems.apothecary_pouch || inSlot.getItem() == ModItems.component_pouch) {
        if (foundPouch) {
          return false;
        }
  ...
  public ItemStack getCraftingResult(InventoryCrafting inv) {
    ...
    for (int i = 0; i < inv.getSizeInventory(); i++) {
      ItemStack inSlot = inv.getStackInSlot(i);
      if (inSlot.getItem() == ModItems.component_pouch || inSlot.getItem() == ModItems.apothecary_pouch) {
        pouch = inSlot;
      } else if (DyeUtils.isDye(inSlot)) {
        dye = inSlot;
      }
