...
public class SpellSanctuary extends SpellBase {
  ...
  public SpellSanctuary(ResourceLocation name) {
    ...
    properties.addProperties(PROP_COOLDOWN, PROP_CAST_TYPE, PROP_COST_1, PROP_COST_2, PROP_VELOCITY, PROP_RADIUS_X, PROP_RADIUS_Y, PROP_RADIUS_Z, PROP_RADIUS_BOOST, PROP_FIRE_DURATION, PROP_LEVITATION_DURATION, PROP_WITHER_AMPLIFIER, PROP_WITHER_DURATION, PROP_SPIDER_DAMAGE, PROP_UNDEAD_DAMAGE);
        ...
        if ((e instanceof IProjectile || (EntityUtil.isHostile(e) && !info.has(UNPEACEFUL))) && (!SpellConfig.spellFeaturesCategory.getSanctuaryBlacklist().contains(EntityList.getKey(e)))) {
          if (e.getDistanceSq(player) < r) {
            ...
            if (e instanceof EntityLivingBase) {
              ...
              if (info.has(WITHER)) {
                ((EntityLivingBase) e).addPotionEffect(new PotionEffect(MobEffects.WITHER, ampInt(wither_duration), wither_amplifier));
              }
              if (info.has(LEVITATE)) {
                ((EntityLivingBase) e).addPotionEffect(new PotionEffect(MobEffects.LEVITATION, ampInt(levitation_duration), 0));
              }
