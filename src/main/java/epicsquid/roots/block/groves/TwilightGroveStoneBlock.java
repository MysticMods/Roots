        ...
        for (int j = -2; j <= 2; ++j) {
          ...
          if (rand.nextInt(32) == 0) {
            for (int k = 0; k <= 1; ++k) {
              ...
              //ClientProxy.particleRenderer.spawnParticle(worldIn, Util.getLowercaseClassName(ParticleLeafArc.class), (double) pos.getX() + 0.5D, (double) pos.getY() + 0.5D, (double) pos.getZ() + 0.5D, (i + rand.nextDouble() - 0.05) * 0.04, -0.0001, (j + rand.nextFloat() - 0.05) * 0.04,                  100, 90 / 255.0, 98 / 255.0, (145 / 255.0) + rand.nextDouble() * 0.05, 1, rand.nextDouble() + 0.5, rand.nextDouble() * 2);
            }
          }
        }
      }
