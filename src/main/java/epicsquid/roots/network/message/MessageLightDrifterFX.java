  ...
  public static class MessageHolder implements IMessageHandler<MessageLightDrifterFX,IMessage>
  {
    ...
    public IMessage onMessage(final MessageLightDrifterFX message, final MessageContext ctx) {
      ...
      for (float i = 0; i < 360; i += random.nextInt(12)){
        ...
        if (random.nextBoolean()){
          ParticleUtil.spawnParticleSmoke(world, x, y, z, vx, 0.125f*(random.nextFloat()-0.5f), vz, SpellRegistry.spell_light_drifter.red1, SpellRegistry.spell_light_drifter.green1, SpellRegistry.spell_light_drifter.blue1, 0.125f, 8f+random.nextFloat()*6f, 80, true);
        }
        else {
          ParticleUtil.spawnParticleSmoke(world, x, y, z, vx, 0.125f*(random.nextFloat()-0.5f), vz, SpellRegistry.spell_light_drifter.red2, SpellRegistry.spell_light_drifter.green2, SpellRegistry.spell_light_drifter.blue2, 0.125f, 8f+random.nextFloat()*6f, 80, true);
        }
      }
      return null;
    }
