          ...
          for (int i = 0; i < positions.size()-1; i ++){
            ...
            for (double j = 0; j < dist; j += 0.15){
              ...
              if (random.nextBoolean()){
                ParticleUtil.spawnParticleStar(world, (float)x, (float)y, (float)z, 0, 0, 0, SpellRegistry.spell_radiance.red1*255.0f,SpellRegistry.spell_radiance.green1*255.0f,SpellRegistry.spell_radiance.blue1*255.0f, 0.75f*(float)(1.0f-alphaDist/totalDist), 3f+3f*random.nextFloat(), 14);
              }
              else {
                ParticleUtil.spawnParticleStar(world, (float)x, (float)y, (float)z, 0, 0, 0, SpellRegistry.spell_radiance.red2*255.0f,SpellRegistry.spell_radiance.green2*255.0f,SpellRegistry.spell_radiance.blue2*255.0f, 0.75f*(float)(1.0f-alphaDist/totalDist), 3f+3f*random.nextFloat(), 14);
              }
            }
          }
        }
